...
echo "Copying writeup Markdown to server..."
# Same mirror semantics as static/: the writeups are read from disk at
# startup (content/ under the service working directory), so a stale or
# missing remote dir would serve writeups that disagree with this deploy.
tar -C content -cf - . | ssh -p "$PORT" "${DEPLOY_USER}@$HOST" \
    "rm -rf $REMOTE_DIR/content && \
     mkdir -p $REMOTE_DIR/content && \
     tar -C $REMOTE_DIR/content -xf -"
