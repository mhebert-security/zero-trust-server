...
/// Parse just the `nonce` field out of a /pow/verify POST body.
/// The full `parse_submission` above is used by the verify path, which needs
/// all three fields; the audit path needs only the nonce to recover when the
/// challenge was issued.
fn parse_nonce(body: &[u8]) -> Option<String> {
    let body_str = std::str::from_utf8(body).ok()?;
    for pair in body_str.split('&') {
        if let Some((k, v)) = pair.split_once('=') && k == "nonce" {
...
/// Milliseconds between the challenge's issuance and now, read from a
/// /pow/verify submission body. The issuance time lives in the nonce
/// (`timestamp.random_hex`), so the caller can compute "how long the puzzle
/// sat open" without the server tracking issued nonces.
///
/// The nonce timestamp has one-second resolution, so the result is the
/// sub-second remainder plus any whole seconds the challenge sat open. The
/// Part 1 spec defines the field as current time minus issue time, which is
/// exactly what this returns. `None` when the body carries no parseable
/// nonce.
pub fn solve_elapsed_ms(body: &[u8]) -> Option<u64> {
    let nonce = parse_nonce(body)?;
    let (timestamp_str, _) = nonce.split_once('.')?;
    let issued_at: u64 = timestamp_str.parse().ok()?;
    let issued_ms = issued_at.saturating_mul(1000);
    Some(current_ms().saturating_sub(issued_ms))
}
...
/// Current Unix timestamp in milliseconds, narrowed to `u64` for the solve
/// timing arithmetic. `as_millis()` yields a `u128`; the cast cannot truncate
/// until the year ~58 million, so this one narrowing lives here.
...
mod tests {
    ...
    #[test]
    fn solve_elapsed_ms_recovers_a_recent_issue_time() {
        set_secret();
        let (nonce, _) = generate_challenge().expect("secret configured → challenge issued");
        let body = format!("nonce={nonce}&nonce_sig=x&candidate=0");
        let ms = solve_elapsed_ms(body.as_bytes()).expect("body carries a nonce");
        // The challenge was issued moments ago, so the span is small. The
        // nonce timestamp has one-second resolution, so the value is the
        // sub-second remainder plus any boundary crossed between issue and
        // read; a two-digit value proves no huge span slipped in.
        assert!(ms < 10_000, "a just-issued challenge reports a small elapsed time");
    }

    #[test]
    fn solve_elapsed_ms_is_none_without_a_parseable_nonce() {
        assert!(solve_elapsed_ms(b"candidate=5").is_none());
        assert!(solve_elapsed_ms(b"nonce=").is_none());
        assert!(solve_elapsed_ms(b"nonce=not-a-number.x").is_none());
        assert!(solve_elapsed_ms(b"").is_none());
    }
