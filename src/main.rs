...
fn main() {
    ...
    // Load the project writeups once at startup: every Markdown file under
    // the writeups directory (default "content", overridable with
    // WRITEUPS_DIR) becomes a /projects/<slug> page. A directory that cannot
    // be read is a configuration error, so the server refuses to start
    // rather than serve dead links from a projects page that claims them.
    let writeups_dir =
        std::env::var("WRITEUPS_DIR").unwrap_or_else(|_| "content".to_string());
    let writeup_count = writeup::init(std::path::Path::new(&writeups_dir))
        .unwrap_or_else(|e| panic!("Failed to load project writeups: {e}"));
    println!("Loaded {writeup_count} project writeups from {writeups_dir}");
